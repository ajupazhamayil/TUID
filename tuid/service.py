...
class TUIDService:
    ...
    def init_db(self, temporal_only=False):
        ...
        while not total.hits:
            ...
            total = self.temporal.search({"size": 0})
        logging.info(
            "--- %.20f seconds initdb temporal---" % (time.time() - start_time)
        )
        ...
        while not total.hits:
            ...
            total = self.annotations.search({"size": 0})
        logging.info(
            "--- %.20f seconds in initdb annotations ---" % (time.time() - start_time)
        )
        with suppress_exception:
    ...
    def insert_tuid_dummy(self, rev, file_name, commit=True):
        ...
        if not self._dummy_tuid_exists(file_name, rev):
            ...
            logging.info(
                "--- %.20f seconds insert_tuid_dummy---" % (time.time() - start_time)
            )
        return MISSING
    ...
    def insert_annotations(self, transaction, values):
        ...
        for data in values:
            ...
            while not self._annotation_record_exists(data[0], data[1]):
                ...
                Till(seconds=0.001).wait()
            logging.info(
                "--- %.20f seconds insert_annotations---" % (time.time() - start_time)
            )
    ...
    def destringify_tuids(self, tuids_string):
        ...
        try:
            ...
            for line in lines:
                ...
                line_origins.append(TuidMap(int(float(tuid)), int(linenum)))
            return line_origins
        except Exception as e:
            Log.error(
    ...
    def _apply_diff(self, transaction, annotation, diff, cset, file):
        ...
        if len(list_to_insert) > 0:
            ...
            for _, inserts_list in jx.groupby(list_to_insert, size=SQL_BATCH_SIZE):
                record = {
                    "_id": inserts_list[2] + inserts_list[1] + str(inserts_list[3]),
                    "tuid": inserts_list[0],
                    "file": inserts_list[1],
                    "revision": inserts_list[2],
                    "line": inserts_list[3],
                }
                self.temporal.add({"value": record})
                self.temporal.refresh()
                ...
                start_time = time.time()
                while (
                    self._get_tuid(inserts_list[1], inserts_list[2], inserts_list[3])
                    == None
                ):
                    Till(seconds=0.001).wait()
                logging.info(
                    "--- %.20f seconds _apply_Diff---" % (time.time() - start_time)
                )
        ...
        for _, part_of_insert in jx.groupby(lines_to_insert, size=SQL_BATCH_SIZE):
            for tuid, f, rev, line_num in list(part_of_insert):
                ...
                while self._get_tuid(f, rev, line_num) == None:
                    ...
                    Till(seconds=0.001).wait()
                logging.info(
                    "--- %.20f seconds insert_tuid_With_duplicates ---"
    ...
    def get_new_lines(self, transaction, line_origins):
        ...
        for line_num, ann_entry in enumerate(line_origins):
            if str(ann_entry) in existing_tuids_tmp:
                existing_tuids[line_num + 1] = copy.deepcopy(
                    existing_tuids_tmp[str(ann_entry)]
                )
